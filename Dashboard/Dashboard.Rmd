...
# Sidebar { .sidebar data-width=314}
-----------------------------------------------------------------------
...
# shiny::sliderInput(
# inputId = "cartons",
# label = h4("Carton Quantity"),
# min = 0,
# max = max(data_all_comm_combined$Act_Ctns),
...
p <- processed_data_filtered_comm_tbl()[,.(quantity_by_week = sum(Act_Ctns)), by = .(Season,ShipWeeks)
             ...
             ][,lbl_txt:= paste("Shipping Week: ",ShipWeeks,"\n","Cartons: ",scales::comma(quantity_by_week))] %>% ggplot(aes(season.week,quantity_by_week))+
...
plotly::ggplotly(p,tooltip = "lbl_txt")
